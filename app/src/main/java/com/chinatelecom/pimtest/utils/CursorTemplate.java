...
import com.chinatelecom.pimtest.interfaces.CursorCallback;
import com.chinatelecom.pimtest.interfaces.Predicate;
...
public class CursorTemplate {
    ...
    public static void eachN(Cursor cursor, Closure<Cursor> closure, int maxReadCount){
        int i = 0;
        if (cursor == null) {
            return;
        }
        try {
            ...
            if (i>=maxReadCount && null != cursor && !cursor.isClosed()) {
                cursor.close();
            }
        }
    }
